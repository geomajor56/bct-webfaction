...
L.Control.GroupedLayers = L.Control.extend({
    options: {
        ...
        var b, c, d = document.createElement("label"), e = this._map.hasLayer(a.layer);
        a.overlay ? a.group.exclusive ? (groupRadioName = "leaflet-exclusive-group-layer-" + a.group.id, b = this._createRadioElement(groupRadioName, e)) : (b = document.createElement("input"), b.type = "checkbox", b.className = "leaflet-control-layers-selector", b.defaultChecked = e) : b = this._createRadioElement("leaflet-base-layers", e), b.layerId = L.Util.stamp(a.layer), b.groupID = a.group.id, L.DomEvent.on(b, "click", this._onInputClick, this);
        ...
        if (f.innerHTML = " " + a.name, d.appendChild(b), d.appendChild(f), a.overlay) {
            ...
            if (!g) {
                g = document.createElement("div"), g.className = "leaflet-control-layers-group", g.id = "leaflet-control-layers-group-" + a.group.id;
                var h = document.createElement("label");
                if (h.className = "leaflet-control-layers-group-label", "" != a.group.name && !a.group.exclusive && this.options.groupCheckboxes) {
                    var i = document.createElement("input");
                    i.type = "checkbox", i.className = "leaflet-control-layers-group-selector", i.groupID = a.group.id, i.legend = this, L.DomEvent.on(i, "click", this._onGroupInputClick, i), h.appendChild(i)
                }
                var j = document.createElement("span");
                j.className = "leaflet-control-layers-group-name", j.innerHTML = a.group.name, h.appendChild(j), g.appendChild(h), c.appendChild(g), this._domGroups[a.group.id] = g
            }
            c = g
        ...
        var a, b, c;
        this_legend = this.legend, this_legend._handlingClick = !0;
        var d = this_legend._form.getElementsByTagName("input"), e = d.length;
        for (a = 0; e > a; a++)b = d[a], b.groupID == this.groupID && "leaflet-control-layers-selector" == b.className && (b.checked = this.checked, c = this_legend._layers[b.layerId], b.checked && !this_legend._map.hasLayer(c.layer) ? this_legend._map.addLayer(c.layer) : !b.checked && this_legend._map.hasLayer(c.layer) && this_legend._map.removeLayer(c.layer));
        this_legend._handlingClick = !1
    }, _onInputClick: function () {
        var a, b, c, d = this._form.getElementsByTagName("input"), e = d.length;
        for (this._handlingClick = !0, a = 0; e > a; a++)b = d[a], "leaflet-control-layers-selector" == b.className && (c = this._layers[b.layerId], b.checked && !this._map.hasLayer(c.layer) ? this._map.addLayer(c.layer) : !b.checked && this._map.hasLayer(c.layer) && this._map.removeLayer(c.layer));
